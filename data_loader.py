...
class DataLoader:
    ...
    def data_read(self, start_id, total_thread):
        ...
        while (sample_id + self.start_file_id) <= self.end_file_id:
            ...
            if len(self.queue) >= self.max_queue_size:
                time.sleep(1)
                continue
            processed_data_path = self.data_path + self.data_file + "_" + str(
                sample_id + self.start_file_id) + '_processed.npz'
            ...
            logging.debug(
                'Finish loading processed data id ' + str(sample_id + self.start_file_id) + ',Time cost = %.4f' % (
                            time.time() - st))
            data_file = (uid_array, item_array, cate_array, shop_array, node_array, product_array, brand_array, \
                         ...
                         neg_history_item, neg_history_cate, neg_history_shop, neg_history_node, neg_history_product,
                         neg_history_brand)
    ...
    def _batch_data(self, data, data_slice, args):
        ...
        if args.long_seq_split and args.search_mode == 'cate':
            ...
            for idx, (left_idx, right_idx) in enumerate(seq_split):
                hist_mask[:, left_idx:right_idx] = ((hist_cate == cate_id[:, None]) & (hist_item > 0))[:,
    ...
    def next(self, args):
        ...
        while data_file_read < total_file_num:
            logging.debug(
                "{0} data file read:{1} queue len {2}".format(self.data_file, data_file_read, len(self.queue)))
            if len(self.queue) == 0:
